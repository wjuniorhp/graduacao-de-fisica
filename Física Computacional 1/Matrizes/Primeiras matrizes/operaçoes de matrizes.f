...
*  Cl udio Mois‚s - Mar‡o de 2010
      ...
      program matrizes
      implicit none
      integer i, j, nx, ny, nk
      parameter (nx = 6, ny = 6, nk = 6)                  ! essa ‚ a ordem da matriz, caso vocˆ queira visualizar com
...
11    FORMAT(1X,6(2X,F6.1))

! Criando matrizes baseadas nos ¡ndices i e j.
     
      open(21,file ='matrizes-1.dat',status='unknown') 
      do i = 1, nx
         do j = 1, ny
            if(i.lt.j) then
              ...
              A(i,j) = 1.  ! Com isso defini-se matrizes identidades (fique a vontade para fazer outras suposi‡äes)
              B(i,j) = 1.
